...
echo "<h2>Setting up Safety Patrol Database</h2>";
...
if ($conn->query($sql) === TRUE) {
 ...
 $sql = "CREATE TABLE IF NOT EXISTS patrol_items (
  id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
if ($conn->query($sql) === TRUE) {
    echo "✅ Table patrol_items created successfully<br>";
} else {
    echo "❌ Error creating table patrol_items: " . $conn->error . "<br>";
}
...
if ($conn->query($sql) === TRUE) {
    echo "✅ Table attachments created successfully<br>";
} else {
    echo "❌ Error creating table attachments: " . $conn->error . "<br>";
}

 $sql = "CREATE TABLE IF NOT EXISTS scores_summary (
  id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  report_id INT UNSIGNED,
  tanggal DATE,
...
if ($conn->query($sql) === TRUE) {
    echo "✅ Table scores_summary created successfully<br>";
} else {
    echo "❌ Error creating table scores_summary: " . $conn->error . "<br>";
}

 $sql = "SELECT * FROM users WHERE username = 'imelda'";
...
if ($result->num_rows == 0) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "✅ Default user 'imelda' created successfully<br>";
    } else {
...
echo "<a href='check_db.php'>Check Database Structure</a>";
